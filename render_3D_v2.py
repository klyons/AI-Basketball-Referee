...
# https://www.youtube.com/watch?v=BwZ7s7DqbFs&list=PL3OV2Akk7XpDjlhJBDGav08bef_DvIdH2&index=2
...
#use corners for bounding box
gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
corners = cv2.goodFeaturesToTrack(gray, 4, 0.01, 10)
corners = np.int0(corners)
pts_src = np.float32([corner.ravel() for corner in corners])
# add top down perpective using cv2 BEV
# Define points for transformation
# Detect bounding box and extract corners as pts_src
pts_src = np.float32([[x1, y1], [x2, y2], [x3, y3], [x4, y4]])  # Based on YOLO bounding box
...
top_down_view = cv2.warpPerspective(image, matrix, (width, height))
